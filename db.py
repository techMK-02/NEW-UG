...
class Database:
    def __init__(self, max_retries: int = 3, retry_delay: float = 2.0):
        ...
        self.temp_indexes: Optional[Collection] = None
    ...
    def _connect_with_retry(self, max_retries: int, retry_delay: float):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.temp_indexes = self.db['temp_indexes']
    ...
    def _create_indexes(self):
        ...
        # Temporary topic index storage indexes
        try:
            self.temp_indexes.create_index(
                [("user_id", 1), ("bot_username", 1), ("channel_id", 1), ("batch_name", 1)],
                unique=True,
                name="topic_index_key"
            )
            index_results.append("topic_index compound")
        except Exception as e:
            print(f"{Fore.YELLOW}⚠ Could not create topic index compound index: {str(e)}{Style.RESET_ALL}")

        try:
            self.temp_indexes.create_index(
                "expiresAt",
                name="topic_index_ttl",
                expireAfterSeconds=0
            )
            index_results.append("topic_index TTL")
        except Exception as e:
            print(f"{Fore.YELLOW}⚠ Could not create topic index TTL: {str(e)}{Style.RESET_ALL}")
...
# API Save Function
    def add_api(api_name: str, api_url: str):
        """Add or update an API in MongoDB."""
        apis = db.apis
        apis.update_one(
            {"name": api_name},
            {"$set": {"url": api_url}},
    ...
    # ===== Temporary Topic Index Storage =====
    def add_topic_index(self, user_id: int, bot_username: str, channel_id: Union[int, str], batch_name: str, topic: str, link: str) -> bool:
        try:
            expires_at = datetime.now() + timedelta(days=7)
            self.temp_indexes.update_one(
                {
                    "user_id": user_id,
                    "bot_username": bot_username,
                ...
                },
                {
                    ...
                    "$addToSet": {"topics": {"topic": topic, "link": link}},
            ...
            print(f"{Fore.YELLOW}⚠ add_topic_index error: {str(e)}{Style.RESET_ALL}")
            return False

    def get_topic_index(self, user_id: int, bot_username: str, channel_id: Union[int, str], batch_name: str) -> Dict[str, str]:
        try:
            doc = self.temp_indexes.find_one({
                "user_id": user_id,
                "bot_username": bot_username,
                "channel_id": channel_id,
                "batch_name": batch_name,
            ...
            return {item.get("topic"): item.get("link") for item in topics if item.get("topic") and item.get("link")}
        except Exception as e:
            print(f"{Fore.YELLOW}⚠ get_topic_index error: {str(e)}{Style.RESET_ALL}")
            return {}

    def clear_topic_index(self, user_id: int, bot_username: str, channel_id: Union[int, str], batch_name: str) -> bool:
        try:
            self.temp_indexes.delete_one({
                "user_id": user_id,
                "bot_username": bot_username,
                "channel_id": channel_id,
                "batch_name": batch_name,
            ...
            print(f"{Fore.YELLOW}⚠ clear_topic_index error: {str(e)}{Style.RESET_ALL}")
            return False
